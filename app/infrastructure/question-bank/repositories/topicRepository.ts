...
export class TopicRepository
  ...
  constructor(model: ModelStatic<TopicModel>, defaultTx?: Transaction) {
    super(model, (r) => r, (c) => c, (u) => u, defaultTx);
  }
  ...
  async create(dto: TopicCreate, tx?: Transaction) {
    try {
      ...
      const topic = await this.model.create({ title: safe.title }, { transaction: this.effTx(tx) });
  ...
  async deleteById(id: string, tx?: Transaction) {
    try {
      ...
      await SubjectTopicModel.destroy({ where: { topicId: id }, transaction: this.effTx(tx) });
  ...
  async existsByTitle(title: string, tx?: Transaction) {
    try {
      const found = await this.model.findOne({ where: { title }, transaction: this.effTx(tx) });
