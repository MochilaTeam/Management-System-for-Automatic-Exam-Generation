...
export class SubjectService {
  ...
  constructor(deps: Deps) { this.repo = deps.repo; }
  ...
  async paginateDetail(criteria: ListSubjects): Promise<{ list: SubjectDetail[]; total: number }> {
  const limit = criteria.limit ?? 20;
  const offset = criteria.offset ?? 0;
  ...
  async get_by_id(id: string) { return this.repo.get_by_id(id); }
