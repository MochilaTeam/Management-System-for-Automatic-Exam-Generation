...
export const createSubjectBodySchema = z.object({
  subject_program: z.string().transform(s => s.trim()).pipe(z.string().min(2)),
  subject_name: z.string().transform(s => s.trim()).pipe(z.string().min(2)),
}).strict();

export const updateSubjectBodySchema = z.object({
  subject_program: z.string().transform(s => s.trim()).pipe(z.string().min(2)).optional(),
  subject_name: z.string().transform(s => s.trim()).pipe(z.string().min(2)).optional(),
}).strict().refine(o => Object.keys(o).length > 0, { message: "Debe enviar al menos un campo." });
...
export const listSubjectsQuerySchema = z.object({
  q: z.string().transform(s => s.trim()).pipe(z.string().min(1)).optional(),
  name: z.string().transform(s => s.trim()).pipe(z.string().min(1)).optional(),
  program: z.string().transform(s => s.trim()).pipe(z.string().min(1)).optional(),
...
export const subTopicDetailSchema = z.object({
