...
import { createStudentCommandSchema, listStudentsQuerySchema, studentIdParamsSchema, updateStudentCommandSchema } from "../../schemas/studentSchema";
import { makeCreateStudentCommand } from "../../../../core/dependencies/user/studentDependencies";
...
export async function listStudent(req: Request, res: Response, next: NextFunction) {
  try {
    ...
    const query = await makeListStudentsQuery();
...
export async function deleteStudent(req: Request, res: Response, next: NextFunction) {
  try {
    const { studentId } = studentIdParamsSchema.parse(req.params);
    await makeDeleteStudentCommand().execute({ id: studentId });
    res.status(204).send();
  } catch (err) { next(err); }
}
